...
@router.message(lambda message: message.text == "Калькулятор" or message.text == "/calc")
async def cmd_menu(message: types.Message, state: FSMContext):
    text1 = "Оберіть потрібну вам величину для розрахунку"

    kb = [
        [KeyboardButton(text="1.Конвертування відстаней")],[KeyboardButton(text="4.Температури")],
        [KeyboardButton(text="2.Швидкості польоту")], [KeyboardButton(text="5.Вага")],
        [KeyboardButton(text="3.Тиск")], [KeyboardButton(text="6.Англійських галонів в літри")],
...
async def process_with_puree(message: types.Message, state: FSMContext):
    text1 = "1Напишіть відстань а вона буде конвертована"
    await asyncio.sleep(1)
...
async def process_with_puree(message: types.Message, state: FSMContext):
    text1 = "2Введіть число а воно буде конвертовано до величини відносно інших одиниць виміру"
...
async def process_with_puree(message: types.Message, state: FSMContext):
    text1 = "3Введіть число а воно буде конвертовано до величини відносно інших одиниць виміру"
...
async def process_with_puree(message: types.Message, state: FSMContext):
    text1 = "4Введіть число а воно буде конвертовано до величини відносно інших одиниць виміру"
...
async def process_with_puree(message: types.Message, state: FSMContext):
    text1 = "5Введіть число а воно буде конвертовано до величини відносно інших одиниць виміру"
...
async def process_with_puree(message: types.Message, state: FSMContext):
    text1 = "6Введіть число а воно буде конвертовано до величини відносно інших одиниць виміру"
...
async def miles_in_cilometrs(message: types.Message, state: FSMContext):
    try:
        number = float(message.text)  # Використовуйте float для більшої точності з числами
        result = {
...
async def tisk_conversion(message: types.Message, state: FSMContext):
    try:
        number = float(message.text)  # Використовуйте float для обробки чисел з дробовою частиною
        result = {
        ...
        await message.answer(f"{number} дюйми рт. ст. = {result['inHg_to_mmHg']} мм рт. ст.")
        await message.answer(f"{number} мм рт. ст. = {result['mmHg_to_inHg']} дюйми рт. ст.")
...
async def volume_conversion(message: types.Message, state: FSMContext):
    try:
        ...
        result = {
            ...
            "gallons_to_us_gallons": number * 1.2205, # Англійські галони в умовні галони
            "us_gallons_to_gallons": number * 0.830   # Умовні галони в англійські галони
        }

        await message.answer(f"{number} англ. галонів = {result['gallons_to_liters']} літрів")
        await message.answer(f"{number} літрів = {result['liters_to_gallons']} англ. галонів")
        await message.answer(f"{number} англ. галонів = {result['gallons_to_us_gallons']} умовних галонів")
        await message.answer(f"{number} умовних галонів = {result['us_gallons_to_gallons']} англ. галонів")
